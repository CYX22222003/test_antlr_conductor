...
class RustCompiler
    ...
    if (stmts !== null) {
      if (Array.isArray(stmts)) {
        for (let i = 0; i <= stmts.length - 1; i++) {
          const stmt = stmts[i];
          this.visit(stmt);
          if (i !== stmts.length - 1) this.instrs[this.wc++] = { tag: "POP" };
        }
      } else {
        this.visit(stmts);
      }
  ...
  public visitFunctionDeclaration(ctx: FunctionDeclarationContext): void {
    ...
    this.compile_time_environment = this.compile_time_environment_extend(
      params,
      current_ce
    );
  ...
  public visitIfStatement(ctx: IfStatementContext): void {
    ...
    if (ctx.altStatement()) this.visit(ctx.altStatement());
  ...
  public visitExpression(ctx: ExpressionContext): void {
    ...
    if (count === 1) {
      if (ctx.BOOL()) {
        this.instrs[this.wc++] = {
          tag: "LDC",
          val: ctx.getChild(0).getText() === "true",
        };
      } else if (ctx.NUMBER()) {
        this.instrs[this.wc++] = {
          tag: "LDC",
          val: parseFloat(ctx.getChild(0).getText()),
        };
      } else if (ctx.IDENT()) {
  ...
  public visitFunctionCall(ctx: FunctionCallContext): void {
    ...
    this.instrs[this.wc++] = {
      tag: "CALL",
      arity: args ? args.expression().length : 0,
    };
  }
